...
Here's a model that is like the previous but adds a second predictor, which is a normal (non-smoothed) regression coefficient that is almost identical to the original `year` information.
...
Extract information about R-hat and the ration of efficient samples with functions `brms::rhat` and `brms::neff_ratio`.
