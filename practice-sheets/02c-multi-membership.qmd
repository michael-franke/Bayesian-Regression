...
Multi-membership models used in cases where group-level effects are plausible but elements do not necessarily belong just to a single group. For example, if we want to include group-level effects for native language, some individuals may have more than one native language. Individuals with more than one native language would belong to multiple groups, whence the term multi-membership models. Moreover, there are cases where membership in a group is a matter of degree: if a grouping of individuals is by "country of residence", for example, some people might spend variable amounts of time in different countries.
...
::: {.callout-caution collapse="false"}
## Exercise 1c: [BONUS] Compare multi-membership model to vanilla
...
Compare the models based on their posterior predicitive adequacy, e.g., using LOO model comparison (if you can).
