...
| ordinal     | $\xi = \text{cumulative-logit}(\eta; {\delta})$ | $y \sim \text{Categorical}({\xi})$  |
...
For all $1 \le i \le k$, we define the probability $p_i$ of category $i$ via the following (so-called *soft-max* operation):
...
Let's still run a multinomial regression model to test address this conjecture.
...
A common choice of link function for this case is the **cumulative logit function** which takes the linear predictor and a vector $\delta$ of $k-1$ thresholds as arguments to return a probability vector, here denoted as $\mathbf{p}$, whose components are defined like so:

$$
p_i = \text{cumulative-logit}(\eta; \delta) = \begin{cases}
\text{logistic}(\delta_1 - \eta) & \text{if } i=1 \\
\text{logistic}(\delta_{i} - \eta) - p_i & \text{if } i>1 \\
\end{cases}
$$ 

To see what is going on, consider the a case with three categories.
Fix the two threshold $\delta_1=-0.75$ and $\delta_2=1.6$ just for illustration.
Now assume that we have a case there the linear predictor value $p$ is zero.
...
If the linear predictor $\xi$ is estimated to be bigger than zero, this intuitively means that we shift all of the threshold to the left (by the same amount).
For example, the plot below shows the case of $\xi=1$ where the probability of the first category decreases while that of the third increases.
...
$$
\begin{align*}
\eta  &= X \beta       && \color{gray}{\text{[linear predictor]}} \\
\xi &= \text{cumulative-logit}(\eta; \delta) && \color{gray}{\text{[predictor of central tendency]}} \\
y & \sim \text{Categorical}(\xi) && \color{gray}{\text{[likelihood]}}
\end{align*}
$$
