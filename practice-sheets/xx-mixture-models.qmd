...
We can also run this finite mixture model in `brms`. We saw earlier already that fitting the parameters of a single Gaussian is like fitting an intercept-only simple linear regression model.
We can add finite mixtures to `brms` by defining setting the `family` parameter to with the help of the `brms::mixture()` function.
