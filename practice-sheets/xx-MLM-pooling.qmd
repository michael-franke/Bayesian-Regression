...
But should we not, somehow, also take into account that these measurements are from different counties? Okay, so let's just calculate the mean log-random measured for each county:
...
Aha! There is a difference between the (empirical) grand mean and the (empirical) mean-of-means. This is because there different numbers of observations for each county:
...
The grand mean does not distinguish at all which observation came from which county. The mean-of-means, on the other hand, puts "counties first", so to speak, and does not acknowledge that the number of observations each county contributed. To understand how this can yield a difference, look at this picture:
...
data_radon |> 
  ...
  annotate("text", x = empirical_mean - 0.4, y = 70, label = "grand mean", color = project_colors[2], size = 8) +
  annotate("text", x = emp_mean_of_means + 0.52, y = 20, label = "mean-of-means", color = project_colors[3], size = 8) +
  theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank())
```

The $x$-position of the dots represents the mean for each county. The size of the dots represents the number of observations for each county. The grand mean (in red) is lower because some "heavier dots pull it to the left". Reversely, the mean-of-means (in yellow) is "pulled more towards the right by the lighter dots" (since it doesn't care about the size of the dots).
...
Suppose we want a Bayesian measure (with quantified uncertainty) of the grand mean and the mean-of-means, how would we do it? (Maybe you want to think about this for a moment, before you uncover the solution below!)

::: {.callout-caution collapse="false"}
## Exercise

Retrieve a Bayesian estimate of the grand mean and of the mean-of-means for the log-randon measure.

::: {.callout-tip collapse="true"}
### Solution

The grand mean is estimable with an intercept-only model.
...
The Bayesian estimate of the grand mean is given by the intercept term:

```{r}
grand_mean_estimate <- 
  fit_InterOnly |> tidybayes::tidy_draws() |> 
  pull(b_Intercept) |> 
  aida::summarize_sample_vector(name = "grand mean")
grand_mean_estimate
```
...
<span style = "color:darkgreen"> add an exercise if you can, please </span>  
