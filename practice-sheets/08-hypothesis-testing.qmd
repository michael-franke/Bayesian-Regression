...
1. Testing with frequentist $p$-values
2. Testing with Bayesian $p$-values
...
# TODO: add picture with colored ROPE
...
# # plot the results
# dens <- density(LH_postPred)
# dens_df <- tibble(x = dens$x, y = dens$y)
# 
# dens_df |> 
#   ggplot(aes(x = x, y = y)) + 
#   geom_ribbon(aes(ymin=0, ymax=y, fill=x < LH_data), alpha=0.3) + # Shade below LH_data
#   scale_fill_manual(values=c("TRUE"=project_colors[2], "FALSE"="transparent")) + # Color the shading
#   geom_line()
  
print(mean(LH_data < LH_postPred))
